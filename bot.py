...
async def handle_auto_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not msg or not msg.text or msg.chat_id != GROUP_CHAT_ID:
        return
    text = msg.text.lower()
    is_q = "?" in msg.text or any(w in text for w in [
        "як", "де", "коли", "розкажи", "підкажи", "допоможи", "не можу",
        "скільки", "як оплатити", "де знайти", "як активувати"
    ])
    if not is_q:
        return
    topics = {
        "підписка": ["підписка", "оплатити", "оплата", "скільки коштує", "поновити"],
        "бот": ["бот", "активувати", "старт", "команди"],
        "нова пошта": ["нова пошта", "відділення", "відправити"],
        "події": ["подія", "зустріч", "захід", "зареєструватись"],
        "день народження": ["день народження", "вішліст", "подарунок", "збір"],
    }
    matched = None
